  ...
  return (
    <ul className="grid gap-2">
      {items.map((item, index) => (
        <li key={`${item}-${index}`} className="rounded-md border bg-muted/20 px-3 py-2 text-sm">
          {item}
        </li>
      ))}
    </ul>
...
export default async function InterviewPrepPage({ params }: InterviewPrepPageProps) {
  ...
  const { appUser, profile, match, interviewPreps } = await getInterviewPrepDetail(matchId);
          ...
          <Card>
            ...
            <CardContent className="grid gap-3 text-sm leading-6 text-muted-foreground">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Badge variant="outline">{answerGuidance.source || "deterministic-baseline"}</Badge>
              </div>
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent className="grid gap-4 text-sm leading-6">
                <div>
                  ...
                  <p className="mt-2 rounded-md border bg-muted/20 px-3 py-2 text-muted-foreground">
                    ...
                    <div className="mt-2 grid gap-2">
                      {(answerGuidance.evidence_to_use ?? []).length > 0 ? (
                        (answerGuidance.evidence_to_use ?? []).map((item) => (
                          <div key={`${item.topic}-${item.framing}`} className="rounded-md border p-3">
                    ...
                    <div className="mt-2 grid gap-2">
                      {(answerGuidance.topics_to_be_careful_with ?? []).length > 0 ? (
                        (answerGuidance.topics_to_be_careful_with ?? []).map((item) => (
                          <div key={`${item.topic}-${item.guidance}`} className="rounded-md border p-3">
                            <p className="font-medium">{item.topic}</p>
                  ...
                  {weakTopics.map((topic) => (
                    <div key={topic.topic} className="rounded-lg border p-3 text-sm leading-6">
                      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                        ...
                        <Badge variant={topic.severity === "Critical" ? "secondary" : "outline"}>
                          {topic.severity}
                        </Badge>
                      </div>
